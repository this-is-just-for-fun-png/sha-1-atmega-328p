...
void print_char(uint32_t *H);

int main()
{
  
   unsigned char msg[120] = "ESY - 220864";
...
void print_char(uint32_t *H)  //übergabe an usart_byte
{
  ...
  for(int i = 0; i < 5; i++)
  {
    ...
    for(int k = 0; k < 4; k++)
    {
      //übergabe an usart_byte
      printf("%02x", (unsigned char) ((H[i] >> (8*(3-k)))));
      printf(" ");
    }
    printf("\n");
  }
